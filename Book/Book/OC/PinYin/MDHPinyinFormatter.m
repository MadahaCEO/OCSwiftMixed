...
+ (NSString *)formatHanyuPinyinWithNSString:(NSString *)pinyinStr
  ...
  if ((ToneTypeWithToneMark == [outputFormat toneType]) && ((VCharTypeWithV == [outputFormat vCharType]) || (VCharTypeWithUAndColon == [outputFormat vCharType]))) {
      @throw [NSException exceptionWithName:@"Throwing a BadMDHHanyuPinyinOutputFormatCombination exception" reason:@"tone marks cannot be added to v or u:." userInfo:nil];
  }
  if (ToneTypeWithoutTone == [outputFormat toneType]) {
     ...
     pinyinStr = [MDHPinyinFormatter convertToneNumber2ToneMarkWithNSString:pinyinStr];
  }
...
+ (NSString *)convertToneNumber2ToneMarkWithNSString:(NSString *)pinyinStr {
  ...
  if ([lowerCasePinyinStr mdh_matchesPatternRegexPattern:@"[a-z]*[1-5]?"]) {
    ...
    NSString *allMarkedVowelStr = @"āáăàaēéĕèeīíĭìiōóŏòoūúŭùuǖǘǚǜü";
      if ([lowerCasePinyinStr mdh_matchesPatternRegexPattern:@"[a-z]*[1-5]"]) {
          ...
          NSInteger tuneNumber = [MDHPinyinFormatter getNumericValue:[lowerCasePinyinStr characterAtIndex:lowerCasePinyinStr.length -1]];
          NSInteger indexOfA = [lowerCasePinyinStr mdh_indexOf:charA];
          NSInteger indexOfE = [lowerCasePinyinStr mdh_indexOf:charE];
          NSInteger ouIndex = [lowerCasePinyinStr mdh_indexOfString:ouStr];
          if (-1 != indexOfA) {
              ...
              for (NSInteger i = [lowerCasePinyinStr length] - 1; i >= 0; i--) {
                  if ([[NSString mdh_valueOfChar:[lowerCasePinyinStr characterAtIndex:i]] mdh_matchesPatternRegexPattern:@"[aeiouv]"]) {
          ...
          if ((defautlCharValue != unmarkedVowel) &&
              ...
              NSInteger rowIndex = [allUnmarkedVowelStr mdh_indexOf:unmarkedVowel];
